# AI 신용평가모형 기반 챗봇 서비스 구현
**대출 신청자의 상환 능력을 정확히 평가하고, 대출 거절에 따른 피드백이 가능한 새로운 개인화 대출 서비스를 구현하였습니다.**

본 프로젝트는 하나금융그룹과 함께하였고, Python 및 Pytorch 환경에서 개발하였습니다.


</br>

<div align="center">
    <img src="https://github.com/juunho/Completed_Projects/assets/81394769/3f73d167-d45c-489f-af4b-34932d02f8f7" width="43%">
</div>

## 개요
~~~
Partner: 하나금융그룹
Date : 2022.10 ~ 2023.06
Team: 연세대 정보대학원 3명, 하나금융티아이 1명, 하나금융지주 1명

My Roles:
- 프로젝트 리딩 및 그룹사 의견 조율
- 딥러닝 기반 대안 신용 평가 모형 구축
- 프롬프트 엔지니어링을 통한 대출 거절 피드백 주입
~~~

</br>

## 문제 정의
> 기존의 대출 서비스는 대출이 거절된 고객에게 적절한 피드백을 제공하지 못했습니다.
>
> 대출 거절에 대한 이유 설명과 대안 상품 추천이 이루어지지 않았고, 이는 부정적인 고객 경험 및 고객 이탈로 이어졌습니다.
>
> **💡 &nbsp;따라서, 저희 팀은 고객 경험 개선을 위한 AI 기반의 개인화된 대출 서비스 '하나 ONE-FIT'을 제안합니다.**



</br>


## 접근 방법

<p align="center">
  <img src="https://github.com/juunho/Completed_Projects/assets/81394769/a5b693a0-1f5f-47a0-bc6e-afd3b0b4550a" width="80%">
</p>

</br>

- **그래프 네트워크 기반의 신용평가 모형 개발을 통한 고객 프로파일링 정밀화**

- **다각화된 XAI 기법, 대출자 간 연결 정보를 활용한 대출 거절 피드백 구현**

- **대화형 AI 프롬프트 엔지니어링을 통한 챗봇 서비스 개발**

</br>

## 상세 내용
**대안 AI 신용평가 모형 개발**
<p align="center">
  <img src="https://github.com/juunho/Completed_Projects/assets/81394769/3e9e757f-9375-4f21-a87b-423ed5c47c83" width="80%">
</p>

</br>

---

**신용평가 결과 바탕의 대출 고객 특성 추론**

<p align="center">
  <img src="https://github.com/juunho/Completed_Projects/assets/81394769/ba1dbb48-9151-49bc-b981-8baf98d17d9b" width="80%">
</p>

</br>

---

**생성형 AI 모델 프롬프트 엔지니어링 수행**

<p align="center">
  <img src="https://github.com/juunho/Completed_Projects/assets/81394769/c1a354d1-f5cb-4797-847b-1e59a918a082" width="80%">
</p>

</br>

---

**대출 거절 피드백을 위한 AI 챗봇 서비스 구현**

<p align="center">
  <img src="https://github.com/juunho/Completed_Projects/assets/81394769/0641d561-4588-423a-8668-a0c8a424de63" width="80%">
</p>

</br>

---

**하나 ONE-FIT 서비스 시연**

<p align="center">
  <img src="https://github.com/juunho/Completed_Projects/assets/81394769/4c88a9bf-3909-4b25-9626-e8913ce1b3c4" width="80%">
</p>



</br>


## 주요 성과
- 그룹 서비스 ‘하나원큐’에 적용 가능한 대출 거절 피드백 챗봇 개발

- 그룹사 내 대안 상품 추천을 통한 고객 락인(Lock-In) 효과 기대

- 상담 서비스 자동화에 따른 연간 약 12만 시간의 상담 시간 절약

</br>



## Lessons Learned

- 컴플라이언스를 고려한 모델 아키텍쳐를 구상할 것



</br>

</br>

##### *저작권으로 인해 해당 프로젝트의 코드와 산출물 공유가 불가하니, 추가 정보나 문의 사항은 연락 주시기 바랍니다.*
